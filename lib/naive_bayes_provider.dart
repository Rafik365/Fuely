...
import 'package:trip_tank_fuely/naive_bayes_service.dart';
...
class NaiveBayesPredictionProvider with ChangeNotifier {
  ...
  final NaiveBayesService _naiveBayesService = NaiveBayesService();
  ...
  // Predict fuel consumption using Naive Bayes
  Future<void> predictFuelConsumption(VehicleModel vehicle, int drivingStyle) async {
    ...
    try {
      ...
      // Use Naive Bayes to predict consumption
      _predictionResult = _naiveBayesService.predictFuelConsumption(
        vehicle,
        _route!,
        drivingStyle,
        weatherData,
  ...
  void addConsumptionFeedback(VehicleModel vehicle, double actualConsumption) {
    if (_route != null) {
      ...
      _naiveBayesService.addTrainingData(
        vehicle,
        _route!,
        weatherData,
        actualConsumption
      ...
      _naiveBayesService.trainModel();
    }
  }
  
  // Update prediction as location changes (for real-time updates)
  Future<void> updatePredictionWithNewLocation(VehicleModel vehicle, int drivingStyle) async {
    ...
    if (_route != null && _route!.startLocation.latitude != _currentLocation!.latitude) {
      await calculateRoute();
      if (_route != null) {
        await predictFuelConsumption(vehicle, drivingStyle);
      }
    }
  }
}
